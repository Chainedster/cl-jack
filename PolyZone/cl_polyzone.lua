...
local function addToComboZone(zone)
    if comboZone ~= nil then
        ...
        comboZone = ComboZone:Create({ zone }, { name = "cl-polyzone" })
        comboZone:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
            if leftZones ~= nil then
              for i = 1, #leftZones do
                TriggerEvent("cl-polyzone:exit", leftZones[i].name, leftZones[i].data)
              end
            end
            if enteredZones ~= nil then
              for i = 1, #enteredZones do
                TriggerEvent("cl-polyzone:enter", enteredZones[i].name, enteredZones[i].data, enteredZones[i].center)
              end
            end
        end, 500)
    end
...
RegisterNetEvent("cl-polyzone:createCircleZone")
AddEventHandler("cl-polyzone:createCircleZone", function(name, ...)
  addCircleZone(name, ...)
end)
...
if GetConvar("sv_environment", "prod") == "debug" then
  RegisterCommand("cl-polyzone:debug", function (src, args)
